...
db <-dbConnect(RMySQL::MySQL(),host="localhost",
               dbname="aaaa",user="root",password="Yet!0902107")
...
```{sql connection=db}
select name from jbitem where supplier=all 
...
```{sql connection=db}
select DISTINCT(c.name) from jbcity c, jbsupplier s where c.id=s.city;
```
